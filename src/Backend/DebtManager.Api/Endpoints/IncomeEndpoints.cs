...
public static class IncomeEndpoints
{
    public static void MapIncomeEndpoints(this IEndpointRouteBuilder app)
    {
        ...
        group.MapPost("/", async (IMediator mediator, [Microsoft.AspNetCore.Mvc.FromBody] DebtManager.Application.Features.Incomes.Commands.AddIncome.AddIncomeCommand command) => 
        {
        ...
        {
            var incomes = await mediator.Send(new DebtManager.Application.Features.Incomes.Queries.GetIncomesByUser.GetIncomesByUserQuery(userId));
        ...
        {
            var result = await mediator.Send(new DebtManager.Application.Features.Incomes.Commands.DeleteIncome.DeleteIncomeCommand(id));
