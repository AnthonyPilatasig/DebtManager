...
public static class DebtEndpoints
{
    public static void MapDebtEndpoints(this IEndpointRouteBuilder app)
    {
        ...
        group
            .MapPost(
                "/",
                async (AddDebtCommand command, IMediator mediator) =>
                {
                    var debtId = await mediator.Send(command);
                    return TypedResults.Created(
                        $"/api/debts/{debtId}",
                        new CreateDebtResponse(debtId)
                    );
                }
            )
            .WithName("AddDebt")
            ...
            .WithDescription(
                "Permite agregar una deuda al portafolio del usuario indicando el tipo, saldo, cuota e interés."
            )
        ...
        group
            .MapGet(
                ...
                {
                    var debts = await mediator.Send(
                        new DebtManager.Application.Features.Debts.Queries.GetDebtsByUserId.GetDebtsByUserIdQuery(
                            userId
                        )
                    );
            ...
            .WithDescription(
                "Devuelve una lista completa de deudas vigentes asociadas a un ID de usuario."
            )
        ...
        group
            .MapPut(
                ...
                async (
                    Guid id,
                    DebtManager.Application.Features.Debts.Commands.UpdateDebt.UpdateDebtCommand command,
                    IMediator mediator
                ) =>
                {
                    if (id != command.Id)
            ...
            .WithDescription(
                "Permite modificar los detalles de una deuda existente como el saldo, nombre o cuota."
            )
        ...
        group
            .MapDelete(
                ...
                async (Guid id, IMediator mediator) =>
                {
                    var success = await mediator.Send(
                        new DebtManager.Application.Features.Debts.Commands.DeleteDebt.DeleteDebtCommand(
                            id
                        )
                    );
                    return success ? TypedResults.NoContent() : Results.NotFound();
            ...
            .WithDescription(
                "Marca la deuda como eliminada sin borrarla físicamente de la base de datos."
            )
            .Produces(StatusCodes.Status204NoContent)
            .ProducesProblem(StatusCodes.Status404NotFound);
