...
public static class CashFlowEndpoints
{
    public static void MapCashFlowEndpoints(this IEndpointRouteBuilder app)
    {
        ...
        .WithDescription("Obtiene la sumatoria de todos los ingresos fijos y le resta las cuotas mínimas mensuales de todas las deudas activas.")
        .Produces<FreeCashFlowResponse>(StatusCodes.Status200OK)
        .ProducesProblem(StatusCodes.Status500InternalServerError);

        group.MapGet("/{userId:guid}/projections", async (Guid userId, IMediator mediator) =>
        {
            var result = await mediator.Send(new DebtManager.Application.Features.CashFlow.Queries.GetProjections.GetProjectionsQuery(userId));
            return TypedResults.Ok(result);
        })
        .WithName("GetProjections")
        .WithSummary("Obtiene planes algorítmicos de desendeudamiento")
        .WithDescription("Aplica las fórmulas de Bola de Nieve y Avalancha a las deudas del usuario.");
    }
}
